...
export default function AuthNavButton({ locale }: { locale: string }) {
    ...
    return (
      <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
        <Link
          ...
          className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-xl bg-deep-teal/70 hover:bg-deep-teal border border-deep-teal hover:border-palm-teal/60 text-xs font-mono text-off-white transition shrink-0 whitespace-nowrap"
        ...
        >
          ...
          <span className="max-w-[70px] xl:max-w-[100px] truncate text-[11px] text-off-white/80">
            {user.email?.split('@')[0]}
          </span>
        </Link>
        ...
        <button
          ...
          className="p-1.5 rounded-lg bg-midnight-teal border border-deep-teal hover:border-rose-500 text-off-white/60 hover:text-rose-400 transition shrink-0"
          title="Sign Out"
          aria-label="Sign Out"
        >
  ...
  return (
    <Link
      href={`/${locale}/login`}
      className="px-3 py-1.5 text-xs font-bold uppercase tracking-wider bg-gradient-to-r from-neon-flamingo to-sunset-orange text-white hover:opacity-90 transition duration-300 rounded-xl shadow-[0_2px_12px_rgba(255,61,129,0.35)] flex items-center gap-1.5 shrink-0 whitespace-nowrap"
    >
